...
class CachedLineByLineTextDataset(LineByLineTextDataset):
    ...
    def __init__(self, tokenizer: PreTrainedTokenizer, file_path: str, block_size: int, local_rank=-1):
        ...
        if cached_file_path.exists():
            ...
            progress_bar = tqdm(total=total_chunks, desc="Encoding texts")

            for i in range(0, len(lines), chunk_size):
                chunk_lines = lines[i:i + chunk_size]
                chunk_encoding = tokenizer.batch_encode_plus(chunk_lines, add_special_tokens=True, max_length=128)
                self.examples.extend(chunk_encoding["input_ids"])
...
class ModelArguments:
    ...
    vocabulary_path: Optional[str] = field(
        default=None, metadata={"help": "Path of the Bert vocabulary you want to use"}
    )
    ingredients_list: Optional[str] = field(
        default=None, metadata={"help": "Path of the ingredient list you want to use"}
    )
...
def main():
    ...
    with Path(model_args.ingredients_list).open() as f:
        used_ingredients = json.load(f)
    tokenizer = BertTokenizer(vocab_file=model_args.vocabulary_path, do_lower_case=False,
                              max_len=128,
                              never_split=used_ingredients)  # For one sentence instruction, longer shouldn't be necessary
