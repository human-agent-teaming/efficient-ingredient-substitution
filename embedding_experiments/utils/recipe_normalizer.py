...
def custom_removel_component(doc):
    ...
    for token in doc:
        ...
        if not in_paranthesis and not token.is_digit and token.text not in words_to_remove and token.lemma_ not in words_to_remove and not has_number(
                token) and \
                token.text[0] != '-':
...
class RecipeNormalizer:
    ...
    def __init__(self, lemmatization_types=None):
        self.model = spacy.load("en_core_web_lg",
                                disable=['parser', 'ner'])  # Disable parts of the pipeline that are not necessary
